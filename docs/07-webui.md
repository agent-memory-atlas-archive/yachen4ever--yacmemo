...
- **自动结案**：提案全部条目执行完成或忽略后，文件头部自动打「> 状态：已结案」标记（并把正文「状态：待裁决」改为「已结案」，git 快照）——`memory_search` 默认不再返回已结案提案（显式 `memory_read` 仍可读），执行类工作不会被重复派发；卡片标题随之出现「（已结案）」。系统与 WebUI 只记录判断、绝不直接改动笔记内容——这是"curator 只提案"铁律的延伸：变更永远走有守卫、有 git 快照的 store 工具语义。
