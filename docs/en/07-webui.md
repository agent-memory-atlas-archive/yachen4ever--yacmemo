...
- Proposals are displayed **structurally** per item (severity / type / notes involved / suggestion); the stats row runs the full state machine (pending → executing → executed / blocked / dismissed) and the selector on the right filters by status;
