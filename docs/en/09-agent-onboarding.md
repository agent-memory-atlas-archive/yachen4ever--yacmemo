...
1. **Session start**: first call `memory_context` for a cold-start review, then show the user a one-line onboarding summary, for example:
   ...
   > ✅ yacmemo memory layer onboarded (user: yachen, contract v0.1.3) — 3 profile/preference entries; 12 active topics: "yacmemo deployment config", "backup strategy", ...; latest audit has no pending issues.
