...
**Settlement reconciliation** (added 2026-09-25, bidirectional self-healing): the closure marker works both ways — an agent may also stamp it by hand with `memory_edit` after confirming every finding is done (markdown is the source of truth). At audit time: (1) findings of a marked proposal that carry only legacy "adopted" rows and no execution events get `executed` backfilled (identity=reconcile, idempotent); (2) when a same-day re-review appends new findings, a stale marker no longer matches reality — it is removed automatically and the status line reset, so the proposal returns to pending and becomes searchable again (it gets re-stamped once the new findings are handled). Findings with no records at all are never backfilled (after a re-review they cannot be told apart from new ones — they stay visible for human judgment). The "verified" closing event applies to D-class issues only; for proposals, done means executed. Finding indices run continuously across the proposal and re-review sections (the re-review section renumbers from 1 in print), keeping `P:<file>:<index>` unique.
