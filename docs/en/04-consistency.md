...
**Settlement reconciliation** (added 2026-09-25): the closure marker works both ways — an agent may also stamp it by hand with `memory_edit` after confirming every finding is done (markdown is the source of truth; **the 0.3.5 promise "both routes count" remains in force** — agents on old sessions without the reporting tool close out this way). At audit time, findings of a marked proposal that lack execution events and are not dismissed get `executed` backfilled (identity=reconcile, idempotent). A stale marker — a same-day re-review appending new findings — is revoked **immediately when the curator appends the re-review section** (status line reset, proposal back to pending and searchable; re-stamped once the new findings are handled). The "verified" closing event applies to D-class issues only; for proposals, done means executed. Finding indices run continuously across the proposal and re-review sections (the re-review section renumbers from 1 in print), keeping `P:<file>:<index>` unique.
