...
**结案调和（2026-09-25 增补）**：结案标记是双向的——agent 也可以在确认全部条目完成后用 `memory_edit` 手工打标（markdown 事实源优先）；审计时为已打标提案中缺执行事件的条目补记 `executed`（identity=reconcile，幂等），覆盖事件机制上线前完成的执行与旧口径「已采纳」条目。「复审通过」封口只作用于 D 类，提案条目完成即已执行。
