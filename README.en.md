...
Python 3.11+ · mcp SDK (FastMCP) · SQLite (FTS5 trigram, WAL) · LanceDB · Qwen3-Embedding-0.6B (any OpenAI-compatible endpoint) · rapidfuzz; WebUI frontend Vue 3 + Naive UI + Vite. The server is a single process; no queues, no graph databases; zero generative LLM in the write path (the curator proposal review is the sole exception, and it only proposes, never executes).
